...
module.exports = function(dbOperations){
	...
	dbOperations.prototype.getGeneDosing = function(gene,user){
		...
		return 	this.find(dbConstants.DRUGS.ALL.COLLECTION,query,undefined,undefined,user).each(function(record){
	...
	dbOperations.prototype.removeEntry = function(oID,type,from,user){
		...
		if (type == 'all'){
			...
			promise = this.findOne(dbConstants.DRUGS.ALL.COLLECTION,query,user).then(function(result){
				if (from == 'Dosing'){
