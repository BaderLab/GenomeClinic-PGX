				...
				if (self.first){
					...
					for (patient in self.patientObj){
						if (self.patientObj.hasOwnProperty(patient)){
							...
							for (var j = 0; j < formatField.length; j++){
								...
								if (formatField[j] == 'GT'){
									if (info.indexOf('.') != -1){
										...
										currDoc['gt'] = {'ref':info[0],'alt':info[1]};
							...
							if (cont) {
								var identifier = line[self.identifier];
								var index = self.patientObj[patient].insertCache.indexOf(identifier);

								if ( index != -1 ){
									var merge = true;
								}	else {
									var merge = false;	
									self.patientObj[patient].insertCache.push(identifier);
									currDoc['identifier'] = identifier;
								}

								for (var field in self.mapper){
								...
								// need to merge multiple docs together
								if (merge) {
									var mask = ['chr','start','end','ref','alt'] // Static content that will Always be present
									var mergeDoc = self.patientObj[patient]['documents'][index];
									for (var field in currDoc){
										if (currDoc.hasOwnProperty(field) && mergeDoc.hasOwnProperty(field) && mask.indexOf(field) == -1){
											if (!isArray(mergeDoc[field])) {
												mergeDoc[field] = new Array(mergeDoc[field])
											}
											//each index corresponds to one genotype.
											mergeDoc[field].push(currDoc[field]);
