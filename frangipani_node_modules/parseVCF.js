...
parseVCF.prototype.parseChunk = function(stringArray){
	...
	var promise = new Promise(function(resolve,reject){
		...
		for (var i=0; i < stringArray.length ; i++ ){
			...
			if (stringArray[i] != "" ){	
				if(stringArray[i].startsWith('##INFO')){
					var line = stringArray[i].toLowerCase().replace(/[\'\"<>]+/," ").split(/[,\s=]+/);
					//console.log(line);
					if (line.indexOf('annovar') != -1){
						var ind = line.indexOf('id') + 1;
						console.log(line[ind]);
						self.mapper['anno'].push(line[ind]);
					}
						...
						if (self.patientObj.hasOwnProperty(patient)){
							for (field in self.mapper.static){
								if (self.mapper.static.hasOwnProperty(field)){
									currDoc[field] = line[self.mapper.static[field]];
								}
							}
							for (var j=0; j < annoList.length; j++){
								if (annoList[j].search(/annovar_date/i) == -1){
						...
						for (var j = 0; j < formatField.length; j++ ){
							...
							if (formatField[i] == 'gt'){
								if (info.indexOf('.') != -1){
									...
									currDoc[formatField[j]] = {'ref':info[0],'alt':info[1]};
	...
	for (var i = 0; i < line.length; i++ ){
		if (line[i] !='.'){
			...
			if (!out.hasOwnProperty(inputLine[0])){
				out[line[0]] = new Array(convertNum(line[1]));
			} else {
				out[line[0]].push(convertNum(line[1]));
			}
		}
	}
	return out;
