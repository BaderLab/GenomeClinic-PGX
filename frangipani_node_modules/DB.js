 ...
 *
 *
 * written by: @Patrick Magee, @Ron Ammar
 * email: patrickmageee@gmail.com, ron.ammar@mail.utoronto.ca
 * 19/12/2014
*/
...
/* Empty db class object which will contain all methods. */
var db = function() {
	this.db = null;
}
 ...
 * in the form:  mongodb://[hostname]:[port]/[database]
...
/* insert up to 1000 documents within an array into a specificed collection buy doing a one time call to
 * the collection. If an error occurs it is returned and the data is not 
 * inserted.
 *
 * the options parameter is a single object with two options:
 * tableName: the collection to insert into
 * documents: an array consisting of document objects to insert into a table
 * 
 * to set a custom _id field simply include '_id' as an attribute within the document 
 * object and set it equal to whataever you want. its important to note that only
 * unique id's will be accepted.
*/
db.prototype.insertMany = function(options){
	var self = this;
	var promise = new Promise(function(resolve,reject){
		if(!options.tableName)
			reject(new ReferenceError("No Table Name Provided"));
