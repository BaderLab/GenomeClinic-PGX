...
/* Default DB parameters. */
	...
	var createInitCollections= function() {
		...
		var promise= new Promise(function(resolve, reject) {
			...
			this.insert(dbConstants.ADMIN_COLLECTION_ID, currentDocument)
...
//=======================================================================================
		...
		return this.insert(dbConstants.PROJECT_COLLECTION_ID, projectInfo).then(function(result){
			if (patientTags) {
				return Promise.each(patientTags, function(patient){
					return self.addProjectToPatient(projectInfo[dbConstants.PROJECT_ID_FIELD],patient,users).catch(function(err){
						console.log(err)
					});
				});
			}	
	...
	this.addProjectToPatient = function(project,patient,users){
		assert.notStrictEqual(db,undefined);
	...
	this.findAllPatients=function(query, readyOnly, options, username){
		assert.notStrictEqual(db, undefined); // ensure we're connected first
		var collectionName = dbConstants.PATIENTS_COLLECTION_ID;
		if (!query)
			var query= {};
