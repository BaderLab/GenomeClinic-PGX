...
var findAllPatients=function(query, readyOnly, options, username){
	assert.notStrictEqual(db, undefined); // ensure we're connected first
	var collectionName = dbConstants.PATIENTS_COLLECTION_ID;
	if (!query)
		var query= {};
...
var findProjects = function(projectName, username){
	...
	return insert(dbConstants.PROJECT_COLLECTION_ID, projectInfo).then(function(result){
		if (patientTags) {
			return Promise.each(patientTags, function(patient){
				return addProjectToPatient(projectInfo[dbConstants.PROJECT_ID_FIELD],patient,users).catch(function(err){
					console.log(err)
				});
			});
		}	
...
var addProjectToPatient = function(project,patient,users){
	assert.notStrictEqual(db,undefined);
	...
	doc['users'] = {$each:users};
	doc = {$addToSet:doc};
	return update(dbConstants.PATIENTS_COLLECTION_ID, query, doc).catch(function(err){err})
}


var removePatientsFromProject = function(project, patients){
	assert.notStrictEqual(db,undefined);
	assert(Object.prototype.toString.call(project) == "[object String]", "Invalid Project Name");
	...
	return update(dbConstants.PATIENTS_COLLECTION_ID, query, doc, options).then(function(result){
		return true;
	})
};


var addPatientsToProject = function(project, patients){
	assert.notStrictEqual(db,undefined);
	assert(Object.prototype.toString.call(project) == "[object String]", "Invalid Project Name");
	assert(Object.prototype.toString.call(patients) == "[object Array]", "Patients must be an array");
	var query = {};
...
var checkInDatabase = function(collection,field,value){
	assert.notStrictEqual(db, undefined);
	...
	var query = {};
	query[field] = value;
	return findOne(collection,query).then(function(result){
		if (result){
			return true;
		} else {
			return false;
...
exports.checkInDatabase = checkInDatabase;
exports.removePatientsFromProject = removePatientsFromProject;
exports.addPatientsToProject = addPatientsToProject;
