...
var addPatient= function(options) {
	...
	var promise= new Promise(function(resolve, reject) {
		 ...
		 * and increment it. */
		findOne(dbConstant.ADMIN_COLLECTION_ID, {})
			.then(function(doc) {
				currentPatientCollectionID= doc[dbConstant.CURRENT_PATIENT_INDEX_FIELD];
				// Add new patient
				if (typeof options === 'object'){
					currentDocument = options;
				} else { 
					currentDocument= {};
					currentDocument[dbConstant.PATIENT_ID_FIELD] = options;
				}

				if (currentDocument[dbConstant.PATIENT_ID_FIELD]===undefined)
				...
				return insert(dbConstant.PATIENTS_COLLECTION_ID, currentDocument);
			}).then(function(result) {
				...
				return update(dbConstant.ADMIN_COLLECTION_ID, {}, {$inc: currentDocument});
			}).then(function(result) {
...
/* Find a single document based on the query. 
 * Returns a promise. */
var find= function(collectionName, query, options) {
	assert.notStrictEqual(db, undefined); // ensure we're connected first
	...
	var promise= new Promise(function(resolve, reject) {
		db.collection(collectionName).find(query,options).toArray(function(err, doc) {
			if (err) {
				reject(err);
			}
			resolve(doc);
...
var findAllPatients=function(){
	assert.notStrictEqual(db, undefined); // ensure we're connected first
	var collectionName = dbConstant.PATIENTS_COLLECTION_ID;
	var options = {'_id':0};
	options[dbConstant.PATIENT_ID_FIELD]=1;	
...
var insertMany = function(options){
	...
	var promise = new Promise(function(resolve,reject){
		...
		db.collection(options.tableName,function(err,collection){
