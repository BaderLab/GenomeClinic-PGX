...
var CURRENT_PATIENT_INDEX_FIELD= "curr_patient_index";
var PATIENT_ID_FIELD= "patient_id";
 ...
 * Returns a promise. */
var createInitCollections= function(callback) {
	...
	currentDocument= {};
	currentDocument[CURRENT_PATIENT_INDEX_FIELD]= 1;
	db.collection(ADMIN_COLLECTION_ID).insert(currentDocument);

	// Create a patient collection and index by unique identifiers.
	// Patient and collection IDs are unique.
	currentDocument= {};
	currentDocument[PATIENT_ID_FIELD]= 1;  // index in ascending order
	db.collection(PATIENTS_COLLECTION_ID).ensureIndex(currentDocument, {unique: true},
		function() {
			var currentDocument= {};
			currentDocument[COLLECTION_ID_FIELD]= -1;  // index in descending order
			db.collection(PATIENTS_COLLECTION_ID).ensureIndex(currentDocument, {unique: true},
				callback);
		});
};
...
var addPatient= function(patientID) {
	 ...
	 * and increment it. */
	db.collection(ADMIN_COLLECTION_ID).findOne({}, function(err, doc) {
		assert.equal(err, null);

		var currentPatientCollectionID= doc[CURRENT_PATIENT_INDEX_FIELD];

		// Add new patient
		currentDocument= {};
		currentDocument[PATIENT_ID_FIELD]= patientID;
		currentDocument[COLLECTION_ID_FIELD]= "p" + currentPatientCollectionID;
		db.collection(PATIENTS_COLLECTION_ID).insert(currentDocument, function(err, result) {
			assert.equal(err, null);
			// Increment patient collection ID only after insert is done
			var currentDocument= {};
			currentDocument[CURRENT_PATIENT_INDEX_FIELD]= 1;  // increment by 1
			db.collection(ADMIN_COLLECTION_ID).update({}, {$inc: currentDocument});
		});
	});
};
		 ...
		 * we need to intialize the DB. */
		 count(systemNamespaces).then(function(result) {
		 	if (!result) { // # of collections in DB is 0
		 		...
		 		createInitCollections(function() {
