	...
	.then(function(result) {
		...
		for (var marker in pgx.pgxCoordinates) {
			if (pgx.pgxCoordinates.hasOwnProperty(marker)) {
				/// BUG
				//////////////Temporary - need to use strings, no parseInt
				// Fix after git branch merged with Patrick's
				(function() {
					var tempCoords= {}
					tempCoords[dbConstants.CHROMOSOME]= parseInt(pgx.pgxCoordinates[marker]["chr"]);
