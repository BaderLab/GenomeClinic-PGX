...
getRouter.use(function(request, response) {
	...
	if (request.baseUrl == "/datasets") {
		...
		promise= dbFunctions.findAllPatients(true, {sort: {"completed": -1}});
	}
...
postRouter.use(function(request, response) {
	...
	if (request.baseUrl == "/callsets/search") {
		...
		var currentPatientID= request.body["patient_id"];
		promise= dbFunctions.getPGXVariants(currentPatientID)
