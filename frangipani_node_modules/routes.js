...
module.exports = function(app,passport,dbFunctions,adminMail,adminPass){
	...
	if (adminMail && adminPass){
		var transporter = nodemailer.createTransport({
			service:'gmail',
			auth:{
				user:adminMail, // this must be set up a new each time.
				pass:adminPass
			}
		});
	}
	...
	app.get('/signup',function(req,res){
		res.sendFile('public/frangipani-signup.html',{root:'.'})
	});
	//urlencodedParser
	...
	app.get('/recover', function(req,res){
		...
		res.sendFile('public/frangipani-recover.html',{root:'.'});
	});
	...
	app.use('/database/find',isLoggedIn, function(req,res){
		var username = req.user[dbConstants.USER_ID_FIELD];
		dbFunctions.findAllPatients(undefined, undefined, undefined, username)
		.then(function(result){
			var fieldsArray = []
			queue.queue.map(function(item){
				fieldsArray = fieldsArray.concat(item['fields']);
	...
	app.post('/database/projects/add',isLoggedIn,function(req,res){
		req.body.users.push(req.user[dbConstants.USER_ID_FIELD]);
	...
	app.post("/patients", isLoggedIn, function(req,res){
		...
		var query = {}
		if (req.body.exclude){
			query[dbConstants.PROJECT_ARRAY_FIELD] = {$nin:[req.body['project']]}

		} else {
			query[dbConstants.PROJECT_ARRAY_FIELD] = req.body['project'];
		}

		dbFunctions.findAllPatients(query,true, {sort: {"completed": -1}}, username)
		.then(function(result){
			res.send(result);
		});
	});
