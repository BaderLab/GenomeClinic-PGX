...
postRouter.use(function(request, response) {
	...
	if (request.baseUrl == "/callsets/search") {
		...
		/////////////////  TESTING RESPONSE TO BROWSER 
		// 				   AND TESTING PANEL ADDING
		var testReturn= {"recieved": true};
		for (var attrname in request.body) { testReturn[attrname] = request.body[attrname]; }
		promise= Promise.resolve(testReturn);

		// PANEL addition testing
		var panelName= request.body["patient_id"];
		dbFunctions.addPanel(
			panelName,
